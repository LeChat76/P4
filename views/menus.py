...
class Menu:
    ...
    def add_player_name(self):
        ...
        """ Request for recording à new player (or not) """
        new_one_player = " "
        while new_one_player.upper() != "O" and new_one_player.upper() != "N":
            while True:
                new_one_player = input("Ajouter un autre joueur au tournoi (O/n)?")
                if not new_one_player:
                    new_one_player = "O"
                if not new_one_player.isalpha():
                    print("Merci de répondre par O ou N uniquement.")
                if new_one_player.isalpha():
                    if new_one_player.upper() == "O":
                        self.add_player_name()
                    elif new_one_player.upper() == "N":
                        break
