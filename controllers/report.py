...
class ReportController:
    ...
    def report_tournament_details():
        ...
        if result[2] and result[3]:
            print(f"Ce tournoi a démarré le {result[2]} et s'est terminé le"
                  f" {result[3]}.")
        elif result[2] and not result[3]:
            print(f"Ce tournoi a démarré le {result[2]} mais n'est pas"
                  f" terminé, {result[5]} round(s) sur {result[4]} ont étés"
                  f" exécutés.")
        elif not result[2]:
    ...
    def report_players_name(self, by_name, by_fname):
        ...
        players_uuid_list = PlayerModel().search_all_players(self.by_name,
                                                             self.by_fname)
    ...
    def report_tournament(display_type=None):
        ...
        matchs_ids_list = TournamentModel()\
            .extract_matchs_uuid_list_of_tournament(selected_tournament_uuid)

        """ extraction of all scores of matchs list associated to the
         selected tournament """
        tournaments_scores = MatchModel().extract_scores(matchs_ids_list)
        tournament_name =\
            TournamentModel().extract_tournament_name(selected_tournament_uuid)
