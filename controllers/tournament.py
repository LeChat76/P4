    ...
    def begin_new_tournament(self):
        ...
        while True:
            ...
            for j in range(int(nb_round)):
                ...
                matchs_list = self.tournament_model.extract_matchs_uuid_list_of_tournament(selected_tournament_uuid)
                ...
                self.tournament_model.store_current_round(selected_tournament_uuid, current_round)
                round_end_date = (datetime.now()).strftime("%d-%m-%Y %H:%M:%S")
                self.tournament_model.store_round_date(selected_tournament_uuid, current_round, round_start_date,
                                                       round_end_date)
                current_round += 1
    ...
    def resume_tournament(self):
        ...
        if not_ended_tournament == "no_result":
            ...
            for j in range(current_round - 1, nb_round):
                ...
                matchs_list = self.tournament_model.extract_matchs_uuid_list_of_tournament(tournament_uuid)
                # create players list from previous matches
                previous_matchs_players_list = self.match_model.create_matchs_players_list(matchs_list)
                # extract previous scores from matches
                previous_scores = self.match_model.extract_previous_scores(players_uuid_list, matchs_list)
                ...
                self.tournament_model.store_current_round(tournament_uuid, current_round)
                round_end_date = (datetime.now()).strftime("%d-%m-%Y %H:%M:%S")
                self.tournament_model.store_round_date(tournament_uuid, current_round, round_start_date,
                                                       round_end_date)
                current_round += 1
                if int(nb_round) >= current_round:
                    choix = self.tournament_view.choice_menu("Continuer l'enregistrement des scores (O/n) ?")
