    ...
    def delete_player(self):
        ...
        while True:
            ...
            if all_player_list == "no_result":
                self.player_view.choice_menu("Aucun joueur, merci d'en créer. Appuyez sur [ENTRER] pour revenir"
                                             " au menu.")
                break
    ...
    def display_player(self):
        ...
        while True:
            ...
            if result == "no_result" and not player_to_display == "display_all":
                choix = self.player_view.choice_menu("Aucun résultat. Recommencer (O/n)? ")
                if choix == "N":
                    break
            elif result == "no_result" and player_to_display == "display_all":
                result = self.player_model.search_all_players()
                if result == "no_result":
                    self.player_view.choice_menu("Liste vide. Veuillez en créer. Appuyez sur [ENTRER] pour revenir"
                                                 " au menu.")
                    break
                if (len(result)) == 1:
