...
class PlayerController:
    ...
    def add_player():
        ...
        new_player = PlayerView().add_player_menu()
    ...
    def delete_player():
        ...
        while True:
            all_player_list = PlayerModel().search_all_players()
            if all_player_list == "no_result":
                PlayerView().choice_menu("Aucun joueur, merci d'en créer."
                                         " Appuyez sur [ENTRER] pour revenir"
                                         " au menu.")
                break
            index_player_to_delete = PlayerView().delete_player_menu(
                all_player_list)
            ...
            print("Utilisateur " + player_to_delete['fname'] + " "
                  ...
                  " supprimé.")
            PlayerModel().delete_player(player_to_delete['player_uuid'])
            choix = PlayerView().choice_menu("Supprimer un autre joueur(O/n)?")
            if choix == "N":
                break
    ...
    def display_player():
        ...
        while True:
            player_to_display = PlayerView().display_player_menu()
            result = PlayerModel().search_player(player_to_display)
            if result == "no_result" and not\
                    player_to_display == "display_all":
                choix = PlayerView().choice_menu("Aucun résultat. Recommencer"
                                                 " (O/n)? ")
                if choix == "N":
                    break
            elif result == "no_result" and player_to_display == "display_all":
                result = PlayerModel().search_all_players()
                if result == "no_result":
                    PlayerView().choice_menu("Liste vide. Veuillez en créer."
                ...
                choix = PlayerView().choice_menu("Faire une autre recherche"
                                                 " (O/n)? ")
                if choix == "N":
                    break
            elif len(result) > 0:
